...
- Read-yourself run-artifact path: `<run-folder>/00-jd-parsed.json`
...
For **resume-update**:
- Inline: the list of available resume variant PDFs from the user's profile
- Read-yourself run-artifact path: `<run-folder>/00-jd-parsed.json`
...
**Inline rule (same as Wave B1):** inline only conversation-derived context. Everything on disk — run artifacts and profile files alike — is passed as a path.
...
For **interview-prep**:
- Read-yourself run-artifact paths: `<run-folder>/00-jd-parsed.json`, `<run-folder>/03-company-research.md`
- Read-yourself path: `${PROFILE_DIR}/skills-experience.md`

For **followup-plan**:
- Inline: the `$SOURCE` tag, today's date
- Read-yourself run-artifact path: `<run-folder>/00-jd-parsed.json`
...
All `_run.json.artifacts` statuses updated. Return control to master-apply.md Step 7 (voice lint safety net).
